...
class State
{
    ...
    public static function saveState(&$state, $stage, $rawId = false)
    {
        ...
        if(isset($_GET['client_id'])) {
            error_log($_GET['client_id']);
            $session->setClientId($_GET['client_id']);
        }else {
            error_log("NO CLIENT ID");
        }
        if(isset($_GET['redirect_uri'])) {
            error_log($_GET['redirect_uri']);
            $session->setRedirectUri($_GET['redirect_uri']);
        }else {
            error_log("NO REDIRECT URI");
        }
